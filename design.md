...
| D3 | UI 层 | 纯 headless | React 组件留产品；panel-host/badge 等 DOM 基础设施进框架 |
...
| 会话层 | session-codec / auth-rest / session-store / me-cache | 近原样（已 DI）；me-cache 键 ns 化 |
| API 传输 | api-client 的 apiFetch 核心 | 端点函数留产品；MeInfo 拆核心（email/credits/avatarUrl/plan）+ 产品扩展 |
...
无 React、无 wxt 运行时依赖；chrome.* 全 DI 结构类型（继承「browser 类型化结构子集」纪律）；DOM 能力（panel-host/badge）不依赖产品栈。devDeps 仅 tsdown/vitest/jsdom/eslint/typescript。
...
`/content` 子结构（D6 抓取目标抽象；现状拆解实证：徽标与通道本就目标无关，图片专用的是发现/资格/抓取/解码四块）：

```
/content
├── badge/      徽标三态机 + 悬停隐现 + 顶层重挂 —— 目标无关（近原样，样式/角位策略注入）
├── handoff/    storage.session 交接通道 —— 框架载荷 {kind,base64,mime,requestedAt} + metadata 透传
├── scan/       目标发现器：页面上找候选目标 —— 可换（图片选择器首发）
├── grab/       抓取器接口 + 实现
│   ├── image/  图片抓取器（page-image 泛化：尺寸资格阈值/canvas 重绘/webp 转码）
│   └── (未来: selection/element/... 随产品加)
└── cdn/        SW 兜底拆两层——通用「扩展权限代理取字节」+ 载荷类型专属解码
```

产品装配：`contentRuntime({ grabber: imageGrabber({...阈值}), badge: {...样式} })`。
...
- **构建**：tsdown（tsup 等价备选）；纯 ESM + 每模块 d.ts；dist 按 `src/<module>/index.ts` 分入口；exports map 八入口 + 根便捷入口（createKit + 全类型）
...
- **框架仓**：模块规格测试（jsdom；panel-host/badge DOM 级）+ example 插件 Chrome 冒烟（手动/可脚本化）+ init.sh
...
| Phase | 内容 | 出口门 |
|---|---|---|
| P0 | 仓 bootstrap：harness 四件套 + tsdown/vitest/eslint 接线 + **example/ 最小示例插件**（createKit 装配 + panel 骨架） | 框架 init.sh 绿 + example 插件 Chrome 手动加载验证（加载成功 + 面板开合 + createKit 产出的键/消息/DOM id 断言） |
| P1 | 确定性核心搬迁：①messaging（新写工厂，最小）②io ③session ④api ⑤panel——逐模块「复制测试改参数化（红）→ 复制代码泛化（绿）→ commit」 | 每模块框架 init.sh 绿 |
| P2 | content 层：badge/handoff/scan 骨架 + Grabber 接口 + image 抓取器 + cdn 双层拆分 | 同上 + example 集成徽标抓图链冒烟 |
| P3 | testing 最小集 + README（装配指南/变更协议/溯源映射表）+ GitHub Packages 发布管线打通 | 0.1.0 可安装（dry-run 验证） |
